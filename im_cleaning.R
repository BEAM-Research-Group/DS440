...
{ 
  ...
  im_JPG = list.files(path = cur_path, pattern = '*.JPG')
  # combine into main list of names
  im_names = c(im_names, im_PNG, im_jpg, im_JPG)
}   # end for (folder in folder_names)

# split into train, validate, and test sets
# get count of all images
len_im = length(im_names)
...
im_names_shuffled = sample(im_names)  # does not overwrite im_names
...
read_convert_save = function(type, im_list)
  ...
  # NOTE: 
    ...
    # I'm pretty sure there's a parameter
  images = vector(mode = "list", length = length(im_names))
  if (length(im_list) > 0)  # checks if there are images
  {
    for (im_ in im_list)  # go through each image
    {
      # get image file
      im_file = system.file(paste0(im_dir, type, '/', im_), package="EBImage") 
