...
model %>% compile(
  ...
  optimizer = optimizer_adam(lr = 0.001, decay = 1e-2/epochs),
  metrics = "accuracy"
)
...
hist = model %>% fit_generator(
  ...
  callbacks = list(
    ...
    callback_model_checkpoint("logs/checkpoints1_model4.h5", save_best_only = TRUE),
    # only needed for visualizing with TensorBoard
    callback_tensorboard(log_dir = "logs")
  )
)
# save results as R object in case plotting crashes session
saveRDS(hist, file = 'model4_history.rds')
